...
def create_seldon_api_testing_file(data: pd.DataFrame, target: str,
    ...
    df_for_json["dtype"] = np.where(
        data.dtypes == np.float, 'FLOAT',
        np.where(data.dtypes == np.int, 'INTEGER', np.NaN))
    df_for_json["ftype"] = np.where(data.dtypes == np.number, 'continuous',
                                    'categorical')
    ranges = [
        _column_range(data[column_name]) for column_name in data.columns.values
    ]
    values = [
        _column_values(data[column_name])
