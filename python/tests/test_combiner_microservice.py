...
def test_aggreate_ok():
    ...
    rv = client.get(
        '/aggregate?json={"seldonMessages":[{"data":{"ndarray":[1]}}]}')
    print(rv)
...
def test_aggreate_bad_messages():
    ...
    client = app.test_client()
    rv = client.get(
        '/aggregate?json={"seldonMessages":[{"data2":{"ndarray":[1]}}]}')
    assert rv.status_code == 400
    j = json.loads(rv.data)
    print(j)
    assert j["status"]["reason"] == "MICROSERVICE_BAD_DATA"
...
def test_aggreate_ok_bindata():
    ...
    bdata_base64_result = base64.b64encode(
        base64.b64encode(bdata)).decode('utf-8')
    print(rv)
    j = json.loads(rv.data)
    print(j)
    assert rv.status_code == 200
