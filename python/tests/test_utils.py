...
def test_create_rest_reponse_nparray():
    ...
    result = scu.construct_response_json(user_model, True, request,
                                         raw_response)
...
def test_create_rest_reponse_text_ndarray():
    user_model = UserObject()
    request_data = np.array([["hello", "world"], ["hello", "another",
                                                  "world"]])
    request = {"data": {"ndarray": request_data, "names": []}}
    (features, meta, datadef,
     data_type) = scu.extract_request_parts_json(request)
