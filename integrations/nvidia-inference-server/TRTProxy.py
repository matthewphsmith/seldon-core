...
class TRTProxy(object):
    def __init__(self, url=None, protocol="HTTP", model_name=None, model_version=1):
        ...
        self.input_name, self.output_name, self.dtype, self.input_dims = parse_model(
            url, self.protocol_id, model_name, False
        )
        self.ctx = InferContext(
            self.url, self.protocol_id, self.model_name, self.model_version, False
        )
