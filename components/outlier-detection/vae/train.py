...
def train(model, X, args):
    ...
    if args.standardized:
        ...
        with open(
            args.save_path + "preprocess_" + args.model_name + ".pickle", "wb"
        ) as f:
    ...
    if args.save:  # create callback
        checkpointer = ModelCheckpoint(
            filepath=args.save_path + args.model_name + "_weights.h5",
            verbose=0,
            save_best_only=True,
...
def run(args):
    ...
    X, _ = generate_batch(data, args.samples, 0.0)
    ...
    if args.continue_training:
        ...
        vae.load_weights(
            args.load_path + args.model_name + "_weights.h5"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--continue_training", default=CONTINUE_TRAINING, action="store_true"
    )
