...
def model(
    ...
    while i < hidden_layers:
        enc_hidden = Dense(
            hidden_dim[i], activation="relu", name="encoder_hidden_" + str(i)
        )(enc_hidden)
        i += 1
    ...
    while i < hidden_layers + 1:
        dec_hidden = Dense(
            hidden_dim[-i], activation="relu", name="decoder_hidden_" + str(i - 1)
        )(dec_hidden)
        i += 1

    outputs = Dense(n_features, activation=output_activation, name="decoder_output")(
        dec_hidden
    )
