...
class OutlierIsolationForest(CoreIsolationForest):
    ...
    def __init__(self, threshold=0.0, model_name="if", load_path="./models/"):

        super().__init__(
            threshold=threshold, model_name=model_name, load_path=load_path
        )
    ...
    def send_feedback(self, X, feature_names, reward, truth):
        ...
        # performance metrics
        scores = performance(
            self._labels, self._predictions, roll_window=self.roll_window
        )
        stats = outlier_stats(
            self._labels, self._predictions, roll_window=self.roll_window
        )
    ...
    def metrics(self):
        ...
        nb_outliers_roll = {
            ...
            "value": self.metric[14],
        }
        nb_labels_roll = {
            "type": "GAUGE",
            "key": "nb_labels_roll",
            "value": self.metric[15],
        }
        nb_outliers_tot = {
            "type": "GAUGE",
            "key": "nb_outliers_tot",
            "value": self.metric[16],
        }
        nb_labels_tot = {
            "type": "GAUGE",
            "key": "nb_labels_tot",
            "value": self.metric[17],
        }
