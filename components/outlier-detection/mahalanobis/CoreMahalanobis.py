...
class CoreMahalanobis(object):
    ...
    def __init__(
        self, threshold=25, n_components=3, n_stdev=3, start_clip=50, max_n=-1
    ):
    ...
    def _get_preds(self, X):
        ...
        coefs = (np.arange(nb) + 1.0) / (np.arange(nb) + n + 1.0)
        new_means = self.mean + coefs.reshape((nb, 1)) * (
            roll_partial_means - self.mean
        )
        new_means_offset = np.empty_like(new_means)
        new_means_offset[0] = self.mean
        new_means_offset[1:] = new_means[:-1]

        coefs = ((n + np.arange(nb)) / (n + np.arange(nb) + 1.0)).reshape((nb, 1, 1))
        B = coefs * np.matmul(
            (Xclip - new_means_offset)[:, :, None],
            (Xclip - new_means_offset)[:, None, :],
        )
        cov_batch = (n - 1.0) / (n + max(1, nb - 1.0)) * self.C + 1.0 / (
            n + max(1, nb - 1.0)
        ) * B.sum(axis=0)
        ...
        coefs = (1.0 / (n + np.arange(nb) + 1.0)).reshape((nb, 1, 1))
        B = coefs * np.matmul(
            (proj_x_clip - proj_means)[:, :, None],
            (proj_x_clip - proj_means)[:, None, :],
        )
        ...
        if self.n > self.start_clip:
            self.clip = [
                self.mean - self.n_stdev * stdev,
                self.mean + self.n_stdev * stdev,
            ]

        # Outlier scores and predictions
        x_diff = proj_x - proj_means
        self.score = np.matmul(
            x_diff[:, None, :], np.matmul(all_C_inv, x_diff[:, :, None])
        ).reshape(nb)
        self.prediction = np.array(
            [1 if s > self.threshold else 0 for s in self.score]
        ).astype(int)
    ...
    def metrics(self):
        ...
        is_outlier = {
            "type": "GAUGE",
            "key": "is_outlier",
            "value": np.mean(self.prediction),
        }
        outlier_score = {
            "type": "GAUGE",
            "key": "outlier_score",
            "value": np.mean(self.score),
        }
        nb_outliers = {
            "type": "GAUGE",
            "key": "nb_outliers",
            "value": int(self.nb_outliers),
        }
        fraction_outliers = {
            "type": "GAUGE",
            "key": "fraction_outliers",
            "value": int(self.nb_outliers) / self.n,
        }
