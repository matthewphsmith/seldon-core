...
def model(
    ...
    dim_check = [
        (idx, dim) for idx, dim in enumerate(decoder_dim) if dim != encoder_dim[-1] * 2
    ]
    if len(dim_check) > 0:
        raise ValueError(
            "\nDecoder (layer,units) {0} is not compatible with encoder hidden "
            "states. Units should be equal to {1}".format(
                dim_check, encoder_dim[-1] * 2
            )
        )
    ...
    decoder_states = []
    for i in range(dec_dim):
        decoder_inference, state_h, state_c = decoder_lstm[i](
            decoder_inference, initial_state=decoder_states_inputs[2 * i : 2 * i + 2]
        )
