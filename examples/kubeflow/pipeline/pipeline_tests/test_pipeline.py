...
def test_pipeline():
    ...
    with runner.isolated_filesystem():
        ...
        with open("raw_text.data", "wb") as f:
            raw_arr = np.array(["hello this is a test", "another sentence to process"])
            dill.dump(raw_arr, f)

        # Test Clean text transformer
        result = runner.invoke(
            clean_cli, ["--in-path", "raw_text.data", "--out-path", "clean_text.data"]
        )
        ...
        result = runner.invoke(
            spacy_cli,
            ["--in-path", "clean_text.data", "--out-path", "tokenized_text.data"],
        )
