...
class MnistFfnn(object):
    ...
    def build_graph(self):
        ...
        with tf.name_scope("dense_1") as scope:
            h1 = Dense(256, activation="relu", W_constraint=maxnorm(3))(inp)
            drop1 = Dropout(0.2)(h1)

        with tf.name_scope("dense_2") as scope:
            h2 = Dense(128, activation="relu", W_constraint=maxnorm(3))(drop1)
            drop2 = Dropout(0.5)(h2)

            out = Dense(self.nb_labels, activation="softmax")(drop2)
    ...
    def fit(
        ...
        tensorboard_logname = self.run_dir + "/{}_{}".format(
            self.model_name, now.strftime("%Y.%m.%d_%H.%M")
        )
...
class MnistConv(object):
    ...
    def build_graph(self):
        ...
        with tf.name_scope("dense") as scope:
            ...
            dense = Dense(128, activation="relu", W_constraint=maxnorm(3))(flat)
            drop_dense = Dropout(0.5)(dense)

            out = Dense(self.nb_labels, activation="softmax")(drop_dense)
...
def main():
    mc = MnistConv()
    mc.fit(X_train, y=y_train, X_test=X_test, y_test=y_test)
