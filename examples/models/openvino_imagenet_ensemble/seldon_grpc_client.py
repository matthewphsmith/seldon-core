...
def grpc_request_ambassador_bindata(
    deploymentName, namespace, endpoint="localhost:8080", data=None
):
    ...
    with open(input_images) as f:
        ...
        for j in range(args.repeats):  # repeat the sequence of requests
            for line in lines:
                ...
                response = grpc_request_ambassador_bindata(
                    "openvino-model", "seldon", endpoint=args.ambassador, data=X
                )
                if args.debug:
                    print(response)
